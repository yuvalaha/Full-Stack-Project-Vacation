...
class VacationLogic:
    ...
    def get_one_vacation(self, vacation_id):
        sql = "SELECT * FROM new_vacations WHERE vacation_id = %s"
        params = (vacation_id, )
    ...
    def add_vacation(self, vacation):
        ...
        sql = "INSERT INTO new_vacations (country_id, description, vacation_start_date, vacation_end_date, price, vacation_image) VALUES (%s, %s, %s, %s, %s, %s)"
    ...
    def get_all_vacations_with_country_name(self):
        sql = "SELECT new_vacations.*, countries.country_name FROM new_vacations JOIN countries ON new_vacations.country_id = countries.country_id ORDER BY `new_vacations`.`vacation_start_date`"
    ...
    def update_vacation(self, vacation):
        ...
        sql = "UPDATE new_vacations SET country_id = %s, description = %s, vacation_start_date = %s, vacation_end_date = %s, price = %s, vacation_image = %s WHERE vacation_id = %s"
        params = (vacation.country_id, vacation.description, vacation.vacation_start_date,
    ...
    def delete_vacation(self, vacation_id):
            ...
            sql = "DELETE FROM new_vacations WHERE vacation_id = %s"
            params = (vacation_id,)
            row_count = self.dal.delete(sql, params)

            sql_update = "UPDATE new_vacations SET vacation_id = vacation_id - 1 WHERE vacation_id > %s"
            ...
            update_sql = "ALTER TABLE new_vacations AUTO_INCREMENT = %s" 
    ...
    def __get_old_image_name(self, vacation_id):
        sql = "SELECT vacation_image FROM new_vacations WHERE vacation_id=%s"
    ...
    def is_vacation_index_exist(self, vacation_id):
        sql = "SELECT CASE WHEN EXISTS(SELECT 1 FROM new_vacations WHERE vacation_id = %s) THEN 1 ELSE 0 END AS IndexExists"
        params = (vacation_id, )
    ...
    def how_many_likes_for_every_vacation(self):
        sql = "SELECT new_vacations.vacation_id, COUNT(likes.user_id) AS total_likes FROM new_vacations LEFT JOIN likes ON new_vacations.vacation_id = likes.vacation_id GROUP BY new_vacations.vacation_id"
        return self.dal.get_table(sql)

    # Return the maximum index of vacation table
    def get_max_vacation_index(self):
        sql = "SELECT MAX(vacation_id) AS max_index FROM new_vacations;"
...
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def get_special_locations(self):
        sql = "SELECT new_vacations.*, countries.country_name FROM new_vacations JOIN countries ON new_vacations.country_id = countries.country_id WHERE new_vacations.country_id = 1 OR new_vacations.country_id = 7 OR new_vacations.country_id = 8 OR new_vacations.country_id = 9 OR new_vacations.country_id = 11 OR new_vacations.country_id = 13 OR new_vacations.country_id = 14 ORDER BY `new_vacations`.`vacation_start_date`"
        return self.dal.get_table(sql)
    
    def get_special_food(self):
        sql = "SELECT new_vacations.*, countries.country_name FROM new_vacations JOIN countries ON new_vacations.country_id = countries.country_id WHERE new_vacations.country_id = 1 OR new_vacations.country_id = 5 OR new_vacations.country_id = 10 ORDER BY `new_vacations`.`vacation_start_date`"
        return self.dal.get_table(sql)
    
    def get_winter_vacations(self):
        sql = "SELECT new_vacations.*, countries.country_name FROM new_vacations JOIN countries ON new_vacations.country_id = countries.country_id WHERE (MONTH(vacation_start_date) = 12 OR MONTH(vacation_start_date) = 1 OR MONTH(vacation_start_date) = 2) AND new_vacations.country_id != 9 ORDER BY `new_vacations`.`vacation_start_date`;"
        return self.dal.get_table(sql)
        
    def get_summer_vacations(self):  
       sql = "SELECT new_vacations.*, countries.country_name FROM new_vacations JOIN countries ON new_vacations.country_id = countries.country_id WHERE MONTH(vacation_start_date) = 6 OR MONTH(vacation_start_date) = 7 OR MONTH(vacation_start_date) = 8  ORDER BY `new_vacations`.`vacation_start_date`"
       return self.dal.get_table(sql)
